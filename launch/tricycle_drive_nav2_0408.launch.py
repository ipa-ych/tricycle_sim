...
def generate_launch_description():
    ...
    nav2_config = os.path.join(get_package_share_directory('tricycle_sim'),'config','navi_tricycle_drive.yaml')
    nav2_node = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(nav2_dir, 'launch', 'navigation_launch.py')
            ),
            launch_arguments={'params_file': '--ros-args --params-file ' + nav2_config}.items()
        )
